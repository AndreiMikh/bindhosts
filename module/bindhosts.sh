...
[ -f $MODDIR/disable ] && {
	...
	string="description=Status: 🔑 Disabled | 📜 $(date) | Systemless Hosts Based Ad-Blocker"
...
if [ ! -w $target_hostsfile ] ; then
	...
	string="description=Status: ⚠️ Unwritable Hosts File | Needs Correction | Systemless Hosts Based Ad-Blocker"
        sed -i "s/^description=.*/$string/g" $MODDIR/module.prop
	return
fi
...
reset() {
	...
	# always restore user's custom rules
	sed 's/#.*//; /^$/d; s/^disabled|//g' $PERSISTENT_DIR/custom*.txt >> $target_hostsfile
        string="description=Status: ♻️ Reset | 📜 $(date) | Systemless Hosts Based Ad-Blocker"
        sed -i "s/^description=.*/$string/g" $MODDIR/module.prop
...
pm path org.adaway > /dev/null 2>&1 && grep -q "generated by AdAway" /system/etc/hosts && {
	# adaway coex
	string="description=Status: ☘️ Active  | 📜 Custom Ad-Blocker: 🕊️ AdAway | Systemless Hosts Based Ad-Blocker"
	sed -i "s/^description=.*/$string/g" $MODDIR/module.prop
...
quick_reset_restore () {
	if [ -f $PERSISTENT_DIR/bindhosts_backup ]; then
		...
		echo "⏺️ Quick Reset Toggled!" 
		# localhost
		printf "127.0.0.1 localhost\n::1 localhost\n" > $target_hostsfile
		# always restore user's custom rules
		sed 's/#.*//; /^$/d; s/^disabled|//g' $PERSISTENT_DIR/custom*.txt >> $target_hostsfile
		string="description=Status: ♻️ Reset | 📜 $(date) | Systemless Hosts Based Ad-Blocker"
		echo "ℹ️ Hosts File Reset!"
	fi
	sed -i "s/^description=.*/$string/g" $MODDIR/module.prop
}
...
update_locales() {
	link1="https://raw.githubusercontent.com/AndreiMikh/bindhosts/bot/locales.zip"
...
install_latest_artifact() {
	...
	# via nightly.link
	latest_zip_url=$(download "https://nightly.link/AndreiMikh/bindhosts/workflows/release/master?preview" | sed 's/>/\n/g; s/</\n/g' | busybox grep -E "^https.*\.zip$" | head -n1)
